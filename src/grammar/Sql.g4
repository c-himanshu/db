...
expr
 ...
 | left=expr oper=( '=' | '==' | '!=' | '<>' | K_IS | K_ISNOT | K_IN | K_LIKE ) right=expr 
 | expr K_AND expr 
 | expr K_OR expr 
 | function_name '(' ( K_DISTINCT? expr ( ',' expr )* | '*' )? ')' 
 | '(' expr ')' 
...
exprWhere
 : literal_value #exprLiter
 | BIND_PARAMETER #exprBind
 | ( ( database_name '.' )? table_name '.' )? column_name #exprCol
 | left=exprWhere oper=( '+' | '-' | '*' | '/' | '%' ) right=exprWhere #exprBinaryMathOp
 | left=exprWhere oper=( '=' | '==' | '!=' | '<>' | '<' | '<=' | '>' | '>=' | K_IS | K_ISNOT | K_IN | K_LIKE ) right=exprWhere #exprCompareOp
 | left=exprWhere oper=K_AND right=exprWhere #exprAnd
 | left=exprWhere oper=K_OR right=exprWhere #exprOr
 | function_name '(' ( K_DISTINCT? exprWhere ( ',' exprWhere )* | '*' )? ')' #exprFunc
 | '(' exprWhere ')' #exprParen
 ;
